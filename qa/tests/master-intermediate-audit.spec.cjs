...
const MASTER_SLUG = process.env.TOTEM_MASTER_SLUG || 'totem-demo-master';
const OUTPUT_DIR = path.join(process.cwd(), 'artifacts', 'master-intermediate-audit');
...
const masterRoutes = [
  { route: 'dashboard', path: `/#/master/${MASTER_SLUG}/dashboard` },
  { route: 'bookings', path: `/#/master/${MASTER_SLUG}/bookings` },
  { route: 'clients', path: `/#/master/${MASTER_SLUG}/clients` },
  { route: 'schedule', path: `/#/master/${MASTER_SLUG}/schedule` },
  { route: 'services', path: `/#/master/${MASTER_SLUG}/services` },
  { route: 'settings', path: `/#/master/${MASTER_SLUG}/settings` },
  { route: 'finance', path: `/#/master/${MASTER_SLUG}/finance` },
  { route: 'finance-money', path: `/#/master/${MASTER_SLUG}/finance/money` },
  { route: 'finance-transactions', path: `/#/master/${MASTER_SLUG}/finance/transactions` },
  { route: 'finance-settlements', path: `/#/master/${MASTER_SLUG}/finance/settlements` },
  { route: 'finance-payouts', path: `/#/master/${MASTER_SLUG}/finance/payouts` }
];
...
function generateCases() {
  ...
  for (let run = 1; run <= 3; run += 1) {
    for (const entry of masterRoutes) {
      cases.push({
        name: `master ${entry.route} run ${run}`,
        route: entry.route,
        run,
        path: entry.path,
        globals: { MASTER_SLUG }
  ...
  cases.push({
    name: 'master root redirect',
    route: 'root-redirect',
    run: 1,
    path: `/#/master/${MASTER_SLUG}`,
    globals: { MASTER_SLUG }
  });
...
function writeReports() {
  ...
  const txtPath = path.join(OUTPUT_DIR, `master-intermediate-summary-${stamp}.txt`);
  const csvPath = path.join(OUTPUT_DIR, `master-intermediate-results-${stamp}.csv`);
  ...
  const txt = [];
  txt.push('TOTEM MASTER INTERMEDIATE AUDIT (ODOO EXCLUDED)');
  txt.push(`Generated: ${new Date().toISOString()}`);
  txt.push(`Base: ${BASE}`);
  txt.push(`Master slug: ${MASTER_SLUG}`);
  ...
  if (failedRuns.length === 0) {
    ...
    for (const row of failedRuns) {
      txt.push(
        `${row.name} | status=${row.status} | issues=${row.issues.join(',') || '-'} | duration=${row.duration}ms | final=${row.finalUrl || '-'}`
      );
  ...
  if (warnedRuns.length === 0) {
    ...
    for (const row of warnedRuns) {
      txt.push(
        `${row.name} | status=${row.status} | issues=${row.issues.join(',') || '-'} | duration=${row.duration}ms | final=${row.finalUrl || '-'} | bodyLength=${row.bodyLength} | divs=${row.divs} | headings=${row.headings} | buttons=${row.buttons}`
      );
      if (row.consoleErrors.length) txt.push(`  consoleErrors: ${row.consoleErrors.join(' || ')}`);
      if (row.ignoredOdooConsoleErrors.length) txt.push(`  ignoredOdooConsoleErrors: ${row.ignoredOdooConsoleErrors.join(' || ')}`);
    }
  ...
  for (const row of slowest) {
    txt.push(
      `${row.name} | status=${row.status} | duration=${row.duration}ms | issues=${row.issues.join(',') || '-'} | final=${row.finalUrl || '-'}`
    );
  }
  ...
  const csvHeader = [
    ...
    'failedRequests',
    'badResponses',
    'ignoredOdooConsoleErrors',
    'ignoredOdooFailedRequests',
    'ignoredOdooBadResponses'
  ];
  ...
  for (const row of RESULTS) {
    csvRows.push(
      [
        ...
        csvEscape(row.bodyLength),
        csvEscape(row.divs),
        csvEscape(row.headings),
        csvEscape(row.buttons),
        csvEscape(row.hasReactCrash),
        csvEscape(row.hasVisibleEmpty),
        csvEscape(row.consoleErrors.join(' || ')),
        csvEscape(row.pageErrors.join(' || ')),
        csvEscape(row.failedRequests.join(' || ')),
        csvEscape(row.badResponses.join(' || ')),
        csvEscape(row.ignoredOdooConsoleErrors.join(' || ')),
        csvEscape(row.ignoredOdooFailedRequests.join(' || ')),
        csvEscape(row.ignoredOdooBadResponses.join(' || '))
      ].join(',')
    );
  }
...
test.describe('TOTEM MASTER INTERMEDIATE AUDIT (ODOO EXCLUDED)', () => {
