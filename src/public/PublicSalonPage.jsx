...
function removeRepeatedPhrase(value, phrase) {
  const normalizedPhrase = normalizeMapQueryValue(phrase);
  if (!normalizedPhrase) {
    return normalizeMapQueryValue(value);
  }
  const pattern = new RegExp(`\\b${escapeRegExp(normalizedPhrase)}\\b`, "gi");
  return normalizeMapQueryValue(value).replace(pattern, "").replace(/\s{2,}/g, " ").trim();
}

function extractStreetHouseAddress(address) {
  const normalized = normalizeMapQueryValue(address);
  if (!normalized) {
    return "";
  }
  ...
  if (streetMatch?.[1]) {
    return normalizeMapQueryValue(streetMatch[1]);
  }
...
function cleanMapAddress(value, salonName = "") {
  const salonPattern = salonName ? escapeRegExp(normalizeMapQueryValue(salonName)) : "";
  let normalized = normalizeMapQueryValue(removeRepeatedPhrase(value, salonName));
  if (!normalized) {
    return "";
  }
  ...
  const primarySegment = normalizeMapQueryValue(normalized.split(",")[0] || normalized);
...
function buildMapSearchQuery(contact = {}, identity = {}) {
  ...
  const address = extractStreetHouseAddress(
    cleanMapAddress(pickFirstString(contact?.address, contact?.full_address, ""), salonName),
  );
  ...
  if (salonName && address && city) {
    return normalizeMapQueryValue(`${salonName}, ${address}, ${city}, ${country}`);
  }
  if (address && city) {
    return normalizeMapQueryValue(`${address}, ${city}, ${country}`);
  }
  if (address) {
    return normalizeMapQueryValue(`${address}, ${country}`);
  }
  return "";
}
        ...
        <section style={{ padding: isMobile ? "12px 0 8px" : "18px 0 12px" }}>
          <div style={container}>
            ...
            >
              ...
              <p style={sectionText}>
                Выберите направление услуги и перейдите к онлайн-записи. Детали,
                время и стоимость можно уточнить при записи.
              </p>
            </div>
        ...
        <section style={{ padding: isMobile ? "12px 0 8px" : "18px 0 12px" }}>
          <div style={container}>
            ...
            >
              ...
              <p style={sectionText}>
                Интерьер, рабочие зоны и детали салона.
              </p>
            </div>
          ...
          <div style={container}>
            <div style={{ ...cardStyle, padding: isMobile ? 16 : 20 }}>
              ...
              >
                ...
                <p style={sectionText}>
                  Коротко о салоне, атмосфере и направлениях услуг.
                </p>
              </div>
              ...
              >
                <div>
                  ...
                  <p style={{ ...sectionText, marginTop: 6 }}>
                    Мастера салона будут отображаться здесь после добавления команды.
                  </p>
                </div>
                <div
                  style={{
          ...
          >
            ...
            >
              ...
              >
                ...
                <p style={sectionText}>
                  Адрес, график и контакты для записи и связи.
                </p>
              </div>
